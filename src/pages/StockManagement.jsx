...
const StockManagement = () => {
  ...
  const fetchItems = async (whId = selectedWarehouseId) => {
    setLoading(true);
    try {
      const url = whId ? `http://127.0.0.1:5000/api/inventory?warehouseId=${whId}` : 'http://127.0.0.1:5000/api/inventory';
  ...
  const fetchSales = async () => {
    try {
      const res = await axios.get('http://127.0.0.1:5000/api/sales', config);
      setSales(res.data);
    } catch (err) {}
  };
  ...
  const handleExecuteTransfer = async (batch) => {
    ...
    try {
      await axios.post('http://127.0.0.1:5000/api/inventory/transfer', {
  ...
  const handleStockAdjustment = async (targetBatchId, currentQty, isAdd) => {
    ...
    try {
      ...
      const res = await axios.put(`http://127.0.0.1:5000/api/inventory/${selectedItem._id}`, updatedPayload, config);
  ...
  const handleDeleteBatch = async (batchId, batchNumber) => {
    ...
    try {
      ...
      const res = await axios.put(`http://127.0.0.1:5000/api/inventory/${selectedItem._id}`, { batches: updatedBatches }, config);
