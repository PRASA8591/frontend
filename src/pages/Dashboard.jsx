    ...
    const fetchData = async () => {
      ...
      try {
        const [itemsRes, statsRes, historyRes] = await Promise.all([
          ...
          axios.get('http://127.0.0.1:5000/api/sales/stats/summary', config).catch(() => ({ data: { totalSales: 0, salesCount: 0, chartData: [] } })),
  ...
  useEffect(() => {
    ...
    if (isBillsView) {
      ...
      if (allBills.length === 0) {
        ...
        axios.get('http://127.0.0.1:5000/api/sales', config)
          .then(res => {
