...
const UsersManagement = () => {
  ...
  const fetchWarehouses = async () => {
    try {
      const res = await axios.get('http://127.0.0.1:5000/api/warehouses', config);
  ...
  const handleCreateUser = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://127.0.0.1:5000/api/users', newUserData, config);
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://127.0.0.1:5000/api/users/${selectedUser._id}`, payload, config);
  ...
  const permissionsConfig = [
    { key: 'dashboard', label: 'Dashboard', isSplit: true, editKey: 'dashboard_edit', viewLabel: 'View Dashboard', editLabel: 'Configure Layout' },
    { key: 'pos', label: 'POS Terminal', isSplit: true, editKey: 'pos_edit', viewLabel: 'View Terminal', editLabel: 'Perform Sales' },
    { key: 'price', label: 'Price Management', isSplit: true, editKey: 'price_edit', viewLabel: 'View Price Lists', editLabel: 'Update Prices' },
    { key: 'reports', label: 'Reports Page', isSplit: true, editKey: 'reports_edit', viewLabel: 'View Reports', editLabel: 'Export/Print' },
    { key: 'audit_logs', label: 'Activity & Login Logs', isSplit: true, editKey: 'audit_logs_edit', viewLabel: 'View Audit Logs', editLabel: 'Export Logs' },
    { key: 'settings', label: 'System Settings', isSplit: true, editKey: 'settings_edit', viewLabel: 'View Settings', editLabel: 'Modify Settings' },
    { key: 'locations', label: 'Locations & Branches', isSplit: true, editKey: 'locations_edit', viewLabel: 'View Locations', editLabel: 'Manage Locations' },
    { key: 'approvals', label: 'PO Approvals', isSplit: true, editKey: 'approvals_edit', viewLabel: 'View Approvals', editLabel: 'Approve/Reject' },
    { key: 'recent_bills', label: 'Recent Bills Tracker', isSplit: true, editKey: 'recent_bills_edit', viewLabel: 'View Bills', editLabel: 'Void/Modify Bills' },
