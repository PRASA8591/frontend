...
const Invoices = () => {
  ...
  const loadInvoicingData = async () => {
    ...
    try {
      const [invRes, custRes, itRes] = await Promise.all([
        axios.get('http://127.0.0.1:5000/api/invoices', config),
        axios.get('http://127.0.0.1:5000/api/customers?type=Seller', config),
  ...
  const handlePublishInvoice = async (e, customStatus = 'draft') => {
    ...
    try {
      await axios.post('http://127.0.0.1:5000/api/invoices', {
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    ...
    try {
      await axios.put(`http://127.0.0.1:5000/api/invoices/${id}/status`, { status: newStatus }, config);
