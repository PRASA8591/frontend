...
const AVAILABLE_PAGES = [
  ...
  { key: 'crm', name: 'Customers (CRM)' },
  { key: 'supply', name: 'Supply Chain' },
  { key: 'invoices', name: 'Invoices' },
  { key: 'users', name: 'System Users' },
  { key: 'reports', name: 'Reports' },
  { key: 'settings', name: 'Settings' },
  { key: 'locations', name: 'Locations & Branches' }
];
...
const LocationsManagement = () => {
  ...
  const { user } = useAuth();
  const isAdmin = user?.role === 'admin' || user?.access?.locations_edit === true;
  ...
  const API_URL = 'http://127.0.0.1:5000/api/warehouses';
  ...
  const handleSubmit = async (e) => {
    ...
    const submitData = {
      ...formData,
      isMain: formData.isMain || formData.code === 'WH-MAIN'
    };
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredLocations.map(wh => {
            const isHQ = wh.isMain || wh.code === 'WH-MAIN';
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] border border-slate-200/50 shadow-3xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            
            {/* Modal Header */}
            <div className="px-8 py-6 bg-slate-900 text-white flex justify-between items-center relative overflow-hidden shrink-0">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
              ...
              {/* Allowed Pages Configuration */}
              <div className="space-y-1.5">
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block pl-1">Allowed Pages / Modules</label>
                <div className="grid grid-cols-2 gap-2 p-4 bg-slate-50 rounded-xl border border-slate-200 max-h-40 overflow-y-auto custom-scrollbar">
                  {AVAILABLE_PAGES.map((page) => {
                    const isChecked = (formData.allowedPages || []).includes(page.key);
                    return (
                      <label key={page.key} className="flex items-center gap-2 text-xs font-bold text-slate-700 cursor-pointer select-none py-1 hover:text-blue-600">
                        <input
                          type="checkbox"
                          checked={isChecked}
                          onChange={(e) => {
                            const currentList = formData.allowedPages || [];
                            const updated = e.target.checked
                              ? [...currentList, page.key]
                              : currentList.filter(k => k !== page.key);
                            setFormData(prev => ({ ...prev, allowedPages: updated }));
                          }}
              ...
              <div className="flex items-center justify-between p-3.5 bg-blue-50/50 rounded-xl border border-blue-100/50">
                <div className="space-y-0.5">
                  <p className="text-xs font-black text-slate-800">Designate as Main HQ</p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Primary Corporate Node</p>
                </div>
                <input
                  type="checkbox"
                  checked={formData.isMain || formData.code === 'WH-MAIN'}
                  disabled={formData.code === 'WH-MAIN'}
                  onChange={(e) => setFormData(prev => ({ ...prev, isMain: e.target.checked }))}
                  className="rounded text-blue-600 focus:ring-blue-500 w-5 h-5 border-slate-300 cursor-pointer disabled:opacity-50"
                />
              </div>
