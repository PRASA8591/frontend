...
const POS = () => {
  ...
  const checkActiveShift = async () => {
    try {
      const res = await axios.get('http://127.0.0.1:5000/api/shifts/active', config);
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://127.0.0.1:5000/api/inventory', config);
  ...
  const fetchCustomers = async () => {
    try {
      const res = await axios.get('http://127.0.0.1:5000/api/customers?type=Customer', config);
  ...
  const handleGetPayment = async () => {
    ...
    try {
      await axios.post('http://127.0.0.1:5000/api/sales', salePayload, config);
  ...
  const handleStartShift = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://127.0.0.1:5000/api/shifts/open', { startFloat: parseFloat(shiftFloatInput) }, config);
  ...
  const handleCreateCustomer = async (e) => {
    ...
    try {
      const res = await axios.post('http://127.0.0.1:5000/api/customers', { title: newCustomerTitle, name: newCustomerName, phone: newCustomerPhone, category: 'Retail', type: 'Customer' }, config);
