...
const Reports = () => {
  ...
  useEffect(() => {
    if (activeTab === 'shifts') {
      fetchShiftsData();
    } else if (activeTab === 'price' || activeTab === 'adjustments' || activeTab === 'direct' || activeTab === 'activity' || activeTab === 'login_history') {
      fetchAuditLogsData();
  ...
  const fetchCoreData = async () => {
    ...
    try {
      const [salesRes, itemsRes] = await Promise.all([
        axios.get('http://127.0.0.1:5000/api/sales', config).catch(() => ({ data: [] })),
        axios.get('http://127.0.0.1:5000/api/inventory', config).catch(() => ({ data: [] }))
      ]);
  ...
  const fetchShiftsData = async () => {
    ...
    try {
      const [shiftsRes, usersRes] = await Promise.all([
        axios.get('http://127.0.0.1:5000/api/shifts', config).catch(() => ({ data: [] })),
        axios.get('http://127.0.0.1:5000/api/users', config).catch(() => ({ data: [] }))
      ]);
  ...
  const fetchAuditLogsData = async () => {
    ...
    try {
      const res = await axios.get('http://127.0.0.1:5000/api/audit', config);
  ...
  // ==========================================
  // ACTIVITY LOGS CALCULATIONS & EXPORTS
  // ==========================================
  const filteredActivityLogs = auditLogsArray.filter(log => {
    const isNotLogin = log.action !== 'LOGIN';
    const matchesSearch = (log.details || '').toLowerCase().includes(activitySearch.toLowerCase()) || 
    ...
    const matchesDate = isDateInRange(log.timestamp);
    return isNotLogin && matchesSearch && matchesDate;
  });

  const exportActivityLogsCSV = () => {
    const headers = ['Timestamp', 'Operator', 'Action', 'Module', 'Details'];
    ...
    exportToCSV(headers, rows, `Activity_Logs_${new Date().toISOString().split('T')[0]}.csv`);
  };

  const printActivityLogsReport = () => {
    printService.a4Document({
      title: 'System Activity Audit Log',
      docNumber: `REP-ACT-${new Date().toISOString().slice(0,10)}`,
      date: new Date().toLocaleDateString(),
      partyLabel: 'REPORT GENERATED BY',
      partyDetails: {
        name: user?.username || 'System Operator',
        email: user?.role || 'Staff',
        phone: `Total logs: ${filteredActivityLogs.length}`
      },
      items: filteredActivityLogs.map((log) => ({
        name: log.details || 'Activity Details',
        sku: log.username || 'Operator',
        qty: 1,
        price: 0,
        total: 0,
        batch: log.action || 'ACTION',
        expiry: new Date(log.timestamp).toLocaleString()
      })),
      summaryFields: [
        { label: 'Activity Logs Count', value: filteredActivityLogs.length, isGrand: true }
      ],
      notes: `System-wide transactional and user action log ledger for selected date range.`,
      settings
    });
  };

  // ==========================================
  // LOGIN HISTORY CALCULATIONS & EXPORTS
  // ==========================================
  const filteredLoginLogs = auditLogsArray.filter(log => {
    const isLogin = log.action === 'LOGIN';
    const matchesSearch = (log.username || '').toLowerCase().includes(loginSearch.toLowerCase()) ||
                          (log.ipAddress || '').toLowerCase().includes(loginSearch.toLowerCase()) ||
                          (log.details || '').toLowerCase().includes(loginSearch.toLowerCase());
    const matchesDate = isDateInRange(log.timestamp);
    return isLogin && matchesSearch && matchesDate;
  });

  const exportLoginHistoryCSV = () => {
    const headers = ['Timestamp', 'Operator', 'IP Address', 'Details'];
    ...
    exportToCSV(headers, rows, `Login_History_${new Date().toISOString().split('T')[0]}.csv`);
  };

  const printLoginHistoryReport = () => {
    printService.a4Document({
      title: 'Operator Login History Ledger',
      docNumber: `REP-LOG-${new Date().toISOString().slice(0,10)}`,
      date: new Date().toLocaleDateString(),
      partyLabel: 'REPORT GENERATED BY',
      partyDetails: {
        name: user?.username || 'System Operator',
        email: user?.role || 'Staff',
        phone: `Total logs: ${filteredLoginLogs.length}`
      },
      items: filteredLoginLogs.map((log) => ({
        name: log.details || 'Login Event',
        sku: log.username || 'Operator',
        qty: 1,
        price: 0,
        total: 0,
        batch: log.ipAddress || 'IP: N/A',
        expiry: new Date(log.timestamp).toLocaleString()
      })),
      summaryFields: [
        { label: 'Login Sessions Count', value: filteredLoginLogs.length, isGrand: true }
      ],
      notes: `Authentication access audits capturing operators login timestamps and source IP addresses.`,
      settings
    });
  };
  ...
  const printActiveReport = () => {
    switch (activeTab) {
      ...
      case 'direct':
        ...
        break;
      case 'activity':
        printActivityLogsReport();
        break;
      case 'login_history':
        printLoginHistoryReport();
        break;
      default:
          ...
          <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-4 flex flex-wrap items-center gap-4 text-white">
            ...
            <button 
              onClick={() => {
                ...
                if (activeTab === 'price' || activeTab === 'adjustments' || activeTab === 'direct' || activeTab === 'activity' || activeTab === 'login_history') fetchAuditLogsData();
        ...
        {(user?.role === 'admin' || user?.access?.audit_logs) && (
          <>
            <button 
              ...
              className={`pb-3 text-xs sm:text-sm font-bold border-b-2 whitespace-nowrap transition-colors flex items-center gap-1.5 ${activeTab === 'activity' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
            >
              <Activity className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Activity Logs
            </button>
            <button 
              onClick={() => setActiveTab('login_history')} 
              className={`pb-3 text-xs sm:text-sm font-bold border-b-2 whitespace-nowrap transition-colors flex items-center gap-1.5 ${activeTab === 'login_history' ? 'border-blue-600 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
            >
              <ShieldCheck className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Login History
            </button>
          </>
        )}
      </div>
          ...
          {adminClearanceError ? (
            ...
            <>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
                <div className="p-6 bg-slate-50/50 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                  <div>
                    <h3 className="text-lg font-black text-slate-800 tracking-tight">System Activity Audit Trail</h3>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Immutable record of data changes, edits, deletions and creations</p>
                  </div>
                ...
                <div className="overflow-x-auto">
                  {loadingAudit ? (
                    <div className="py-24 text-center text-slate-400 animate-pulse">Syncing Activity Logs...</div>
                  ) : (
                    <table className="w-full text-left">
                      <thead className="bg-slate-50 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                        <tr>
                      ...
                      <tbody className="divide-y divide-slate-100 text-sm">
                        {filteredActivityLogs.map(log => (
                          <tr key={log._id} className="hover:bg-slate-50/50">
                            ...
                            <td className="px-6 py-4">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase border ${
                                log.action?.includes('DELETED') 
                                  ...
                                  : log.action?.includes('CREATED') || log.action?.includes('ADD')
                            ...
                            <td className="px-6 py-4 text-xs font-bold text-slate-650 uppercase">{log.module}</td>
                            <td className="px-6 py-4 text-slate-600 font-medium">{log.details}</td>
                          </tr>
                        ))}
                        {filteredActivityLogs.length === 0 && (
                          <tr>
                            <td colSpan="5" className="px-6 py-12 text-center text-slate-400 font-bold">No system activity logged for the selected filter.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
          ...
          {adminClearanceError ? (
            ...
            <>
              ...
              <div className="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
                <div className="p-6 bg-slate-50/50 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                  <div>
                    <h3 className="text-lg font-black text-slate-800 tracking-tight">Operator Login History</h3>
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Audit log of system authorization and operator sessions</p>
                  </div>
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    ...
                    <button 
                      ...
                      className="flex items-center gap-1.5 bg-emerald-650 hover:bg-emerald-750 text-white font-bold py-2 px-4 rounded-xl shadow-md text-xs cursor-pointer"
                    >
                      <Download className="w-3.5 h-3.5" /> Export
                    </button>
                  </div>
                ...
                <div className="overflow-x-auto">
                  {loadingAudit ? (
                    <div className="py-24 text-center text-slate-400 animate-pulse">Syncing Login Logs...</div>
                  ) : (
                    <table className="w-full text-left">
                      <thead className="bg-slate-50 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                        <tr>
                          <th className="px-6 py-4">Timestamp</th>
                          <th className="px-6 py-4">Operator</th>
                          <th className="px-6 py-4">Source IP Address</th>
                      ...
                      <tbody className="divide-y divide-slate-100 text-sm">
                        {filteredLoginLogs.map(log => (
                          <tr key={log._id} className="hover:bg-slate-50/50">
                            <td className="px-6 py-4 text-xs font-mono text-slate-500">{new Date(log.timestamp).toLocaleString()}</td>
                            <td className="px-6 py-4 font-bold text-slate-800">{log.username}</td>
                            <td className="px-6 py-4 text-xs font-mono text-slate-650 font-bold">{log.ipAddress || 'N/A'}</td>
                            <td className="px-6 py-4 text-slate-600 font-medium">{log.details}</td>
                          </tr>
                        ))}
                        {filteredLoginLogs.length === 0 && (
                          <tr>
                            <td colSpan="4" className="px-6 py-12 text-center text-slate-400 font-bold">No login sessions logged for the selected filter.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
