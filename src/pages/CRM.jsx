...
const CRM = () => {
  ...
  const handleDepositSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`http://127.0.0.1:5000/api/customers/${activeDetailCustomer._id}/deposit`, {
  ...
  const fetchCustomers = async () => {
    ...
    try {
      const [custRes, invRes, poRes, grnRes] = await Promise.all([
        axios.get('http://127.0.0.1:5000/api/customers', config),
        axios.get('http://127.0.0.1:5000/api/invoices', config).catch(() => ({ data: [] })),
        axios.get('http://127.0.0.1:5000/api/supply/po', config).catch(() => ({ data: [] })),
        axios.get('http://127.0.0.1:5000/api/supply/grn', config).catch(() => ({ data: [] }))
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCustomer) {
        await axios.put(`http://127.0.0.1:5000/api/customers/${editingCustomer._id}`, formData, config);
        toast.success('Entity updated.');
      } else {
        await axios.post('http://127.0.0.1:5000/api/customers', formData, config);
  ...
  const handleDelete = async (customer) => {
    ...
    if (isConfirmed) {
      try {
        await axios.delete(`http://127.0.0.1:5000/api/customers/${customer._id}`, config);
