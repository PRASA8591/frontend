...
const getFirstAllowedPage = (user) => {
  if (!user) return '/login';
  const currentWarehouse = user.currentWarehouse;
  if (user.role === 'admin') {
    if (!currentWarehouse || !currentWarehouse.allowedPages || currentWarehouse.allowedPages.includes('dashboard')) {
      return '/dashboard';
    }
  }
  const menuItems = [
    ...
    { path: '/supply', accessKey: 'supply' },
    { path: '/invoices', accessKey: 'invoices' },
    { path: '/users', accessKey: 'users' },
    { path: '/reports', accessKey: 'reports' },
    { path: '/locations', accessKey: 'settings' },
    { path: '/settings', accessKey: 'settings' }
  ];
  for (const item of menuItems) {
    const hasRoleAccess = user.role === 'admin' || (user.access && user.access[item.accessKey] !== 'none' && user.access[item.accessKey] !== false);
    let hasLocationAccess = !currentWarehouse || !currentWarehouse.allowedPages || currentWarehouse.allowedPages.includes(item.accessKey);
    if (user.role === 'admin' && (item.accessKey === 'settings' || item.accessKey === 'dashboard')) {
      hasLocationAccess = true;
    }
    if (hasRoleAccess && hasLocationAccess) {
      return item.path;
    }
  }
  return '/dashboard';
...
const PrivateRoute = ({ children, accessKey, requiredLevel = 'view' }) => {
  ...
  // Check location allowed pages
  const currentWarehouse = user?.currentWarehouse;
  if (currentWarehouse && currentWarehouse.allowedPages && accessKey) {
    const isAdminOverride = user?.role === 'admin' && 
      (accessKey === 'settings' || accessKey === 'dashboard' || accessKey === 'activation');
      
    if (!isAdminOverride && !currentWarehouse.allowedPages.includes(accessKey)) {
