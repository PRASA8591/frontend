...
const Layout = ({ children }) => {
  ...
  const handleSwitchLocation = async (warehouseId) => {
    try {
      ...
      const res = await axios.post('http://127.0.0.1:5000/api/auth/switch-location', { warehouseId }, {
  ...
  const startProgressUpdate = async () => {
    ...
    try {
      const token = sessionStorage.getItem('token');
      await axios.post('http://127.0.0.1:5000/api/settings/trigger-stock-update', {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
    } catch (e) {
  ...
  const menuItems = [
    ...
    { path: '/locations', name: 'Locations & Branches', icon: <MapPin className="w-5 h-5" />, accessKey: 'locations' },
  ...
  const filteredMenuItems = menuItems.filter(item => {
    ...
    if (user.role === 'admin') {
      ...
      hasRoleAccess = user.access && user.access[item.accessKey] !== 'none' && user.access[item.accessKey] !== false;
    }
    ...
    if (currentWarehouse && currentWarehouse.allowedPages) {
      // Allow settings, locations, dashboard, and activation for admin always so they can configure
      if (user.role === 'admin' && (item.path === '/locations' || item.path === '/settings' || item.path === '/activation' || item.path === '/dashboard')) {
        return true;
      }
      return currentWarehouse.allowedPages.includes(item.accessKey);
    }
              ...
              <div className="relative">
                ...
                >
                  ...
                  <span className="max-w-[120px] truncate">
                    {user.currentWarehouse?.isMain ? '★ ' : ''}
                    {user.currentWarehouse?.name || 'Main HQ'}
                  </span>
                      ...
                      <div className="max-h-60 overflow-y-auto custom-scrollbar">
                        {user.allowedWarehouses.map((wh) => {
                          ...
                          return (
                            ...
                            >
                              <span>{wh.isMain ? '★ ' : ''}{wh.name} ({wh.code})</span>
